...
uuid=6aa92ce2-34be-4ce4-af5c-67bbba296688	#Reno---用户ID   uuid=$(cat /proc/sys/kernel/random/uuid)
...
v2ray_config() {
	...
	while :; do
		...
		# [ -z "$v2ray_transport" ] && 
		v2ray_transport=1 #Reno---默认协议=TCP
...
shadowsocks_config() {
	...
	while :; do
		...
		install_shadowsocks="n" #Reno---不配置
